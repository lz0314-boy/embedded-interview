  ...
  },
  {
    ...
    questions: [
      ...
      },
      {
        ...
        brief: "关闭开关只阻止新存储报警识别，原有活动报警状态仍在；最终在 .so 业务逻辑中新增关闭前清除操作，让 Web 收到清除后的状态。",
        a: `<p><strong>建议按现象→假设→证据→修复回答：</strong></p><p>“测试部反馈关闭存储报警后，新的存储报警不再识别，但之前叠加在界面上的报警信息仍然存在。首先通过 Bug Skill 拉取 Bug 描述、复现步骤、版本和截图/日志，确认问题不是配置没有生效，而是‘禁止继续产生’和‘清除当前已显示状态’两个语义没有同时处理。”</p><p>“我沿着配置加载、存储状态检测、报警状态管理和 Web 状态更新逐层确认，发现原逻辑只在关闭后停止新的存储报警识别，没有清理已经保存的活动报警状态。因此最终修复放在 <code>.so</code> 业务逻辑：检测到存储报警从启用变为关闭时，先清除已有的存储报警状态，再继续后续逻辑。Web 层接收到更新后的状态后，叠加信息就会正常消失，不需要把根因误判成前端绘制问题。”</p><p>“验证覆盖：先触发并显示存储报警，再关闭开关确认旧叠加消失且不再新增；切回开启后重新触发；同时存在其他报警时只清除存储报警；重启、配置持久化和挂载/正式包都要回归。最后把自测结果、Bug ID、版本和测试证据关联后上库。”</p>`,
        followups: [{ q: "为什么关闭开关后旧报警不会自动消失？", a: "原开关只参与后续检测条件，已经进入业务活动状态的报警不会自动回收；关闭动作必须显式清除对应状态，Web 才能根据新状态移除叠加信息。" }, { q: "为什么最终改 .so 而不是 .js？", a: "业务层才拥有存储报警的权威状态和配置变化逻辑，问题根因在关闭时没有清理活动状态；Web 层只是展示状态，改页面只能掩盖问题，刷新或其他入口仍可能复现。" }, { q: "如何避免修复影响其他报警？", a: "只清除存储报警对应的状态，不清空全部报警；回归其他报警叠加、同时触发、重复关闭和重新开启。" }],
        evidence: "本人确认的测试部 Bug 现象与 Skill 辅助定位流程",
        boundary: "已确认修复位于 .so 业务逻辑；具体函数名和状态结构不在公开题库中展开。",
