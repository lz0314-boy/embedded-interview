  ...
  },
  {
    ...
    questions: [
      ...
      },
      {
        ...
        a: `<p>“当前基线是 TESSY 5.1.16、Linux 和已有单元测试工程。原流程依赖 GUI，重复执行耗时且容易漏步骤。我主导探索 CLI 化并封装成 Skill：先把工程路径、测试对象、桩/测试数据、工具环境、报告目录和超时写进配置；流程从创建或复用 Module 开始，完成 Analyze、范围筛选、逐函数用例处理与执行，最后导出报告、覆盖率和审计证据。”</p><p>“目前已经验证 Module 创建、Analyze、简单 C/C++、XML Stub、Fault Injection、单对象执行、覆盖率和报告；自动化核心、范围控制、实例锁和恢复正在建设，MCP 接入是后续阶段。每一步检查进程退出码、关键日志和产物，但最终结论以 TESSY Details 报告为准。”</p><p>对 C0/C1/MC/DC 的回答要谨慎：覆盖率只能说明执行情况，不能替代需求映射、异常输入和桩行为审查。</p>`,
        ...
        evidence: "本人确认的 TESSY Skill；06-tessy-cli-executive-architecture.md",
        boundary: "区分已验证、建设中和待建设能力；覆盖率阈值与内部命令不写入公开题库。",
      ...
      },
      {
        ...
        brief: "统一接入层、自动化核心、TESSY 执行层和结果交付层分离；配置与预置资产复用，阶段可恢复、产物可追溯。",
        ...
        evidence: "06-tessy-cli-executive-architecture.md：TESSY 5.1.16/Linux 总体架构",
        boundary: "Skill 已作为统一入口；自动化核心仍在建设，MCP 和复杂 C++ 通用方案尚未完成。",
        caution: "脚本成功退出不等于测试通过，要同时检查报告状态和失败用例。"
      },
      {
        id: "intern-ruiming-tessy-scope", priority: "must", difficulty: "advanced", tags: ["锐明技术", "TESSY", "范围控制", "C++"],
        q: "为什么 TESSY 自动化要限制只测试目标源文件定义的函数？",
        brief: "避免把头文件内联、模板实例或其他源文件函数误纳入任务，保证测试范围、报告和需求责任边界一致。",
        a: `<p>“自动化不能只把 Analyze 找到的所有函数全部执行。任务先明确目标源文件，再结合 TESSY 的函数信息和编译器/源码解析确认函数定义归属，只保留目标文件实际定义的函数；头文件内联、模板实例、外部库和其他源文件实现要单独标记或排除。”</p><p>“这样做有三个价值：报告数量能和任务范围对上，避免把别人的函数结果算到当前改动上；测试失败时责任边界清楚；复杂 C++ 场景可以明确降级，而不是用错误范围掩盖工具能力不足。范围控制本身也要输出清单和排除原因，方便审计。”</p>`,
        followups: [{ q: "为什么不能只按函数名过滤？", a: "不同源文件可能有同名静态函数，模板/重载还会产生多个实体；必须结合定义文件、签名和编译信息，而不是只看字符串。" }, { q: "范围控制失败会有什么后果？", a: "会漏测目标函数或误报覆盖率，报告无法支撑提测结论，自动修复还可能修改错误文件。" }],
        evidence: "TESSY CLI 架构文档：范围控制、函数定义归属和复杂 C++ 降级方案",
        boundary: "当前文档把范围控制列为建设重点，面试时区分已验证能力与建设中能力。",
        caution: "不要把‘Analyze 能看到函数’说成已经完成了可靠归属校验。"
      },
      {
        id: "intern-ruiming-tessy-result-gate", priority: "must", difficulty: "advanced", tags: ["锐明技术", "TESSY", "报告", "结果门禁"],
        q: "为什么 TESSY CLI 不能只看命令返回码判断测试通过？",
        brief: "进程退出码只说明 CLI 调用层是否结束，最终结论必须读取 TESSY Details 报告、用例状态、覆盖率和失败摘要。",
        a: `<p>“CLI 进程返回 0 只能说明命令执行完成，不能证明每个测试用例通过；反过来，工具异常也可能生成不完整报告。因此结果门禁至少检查：Details 报告是否生成且可解析、测试对象数量是否符合范围、每个用例的 Pass/Fail/Error 状态、覆盖率和未执行原因，以及日志中是否有环境/许可证错误。”</p><p>“只有报告状态和范围校验都通过，验证编排器才把阶段标记为成功；否则保留原始日志和报告，把任务交给用户决策或进入恢复流程。”</p>`,
        followups: [{ q: "覆盖率达到阈值就能提测吗？", a: "不能。还要看需求映射、异常路径、桩行为、编译告警和测试是否真正执行；覆盖率是门槛信号，不是质量证明。" }, { q: "报告缺失但退出码为 0 怎么办？", a: "判定为结果不可信，检查工作目录、许可证、进程残留和磁盘权限，不能生成一份空报告冒充通过。" }],
        evidence: "TESSY CLI 架构文档：结果可信原则和 Details 报告门禁",
        boundary: "具体报告 XML 字段和阈值按项目版本配置，不补写未确认的字段名。",
        caution: "把‘命令成功’和‘测试通过’明确分开。"
      },
      {
        id: "intern-ruiming-tessy-recovery", priority: "should", difficulty: "advanced", tags: ["锐明技术", "TESSY", "恢复", "实例锁", "审计"],
        q: "TESSY 自动化如何处理并发运行和中途失败？",
        brief: "用独立 workspace/账号、实例锁和 Module 基线隔离任务；写操作前保存状态，失败后恢复或隔离异常实例，保留完整证据。",
        a: `<ol><li>每个任务分配独立 workspace、实例标识和输出目录，避免多个任务改写同一 Module。</li><li>创建/修改 Module 等重要写操作前保存基线或快照，获得实例锁后才执行。</li><li>失败时先判断是瞬态环境错误、许可证/进程残留还是测试真实失败；只对幂等技术错误有限重试。</li><li>无法恢复时隔离异常实例，保留命令、日志、报告、配置和最后阶段，不污染下一次任务。</li><li>释放锁和清理进程要放在 finally/退出钩子中，最终报告标出恢复动作和未完成步骤。</li></ol>`,
        followups: [{ q: "为什么不能多个任务共用一个 workspace？", a: "Module、测试数据和中间产物可能互相覆盖，导致报告与源码不匹配；并发隔离比事后猜测污染来源更可靠。" }, { q: "什么时候应该请求人工决策？", a: "真实用例失败、多次修复无效、范围归属不确定或需要改变基线时停止自动化，把证据和选项交给用户。" }],
        evidence: "TESSY CLI 架构文档：实例锁、基线、恢复和审计能力",
        boundary: "文档将自动化核心和恢复列为建设中，回答时不要说已具备完整生产级容错。",
        caution: "自动重试不能掩盖真实测试失败，也不能无保护地覆盖共享 Module。"
      },
      {
        id: "intern-ruiming-skill-mcp", priority: "should", difficulty: "advanced", tags: ["锐明技术", "Skill", "MCP", "AI架构"],
        q: "TESSY Skill、自动化核心和 MCP 分别是什么关系？",
        brief: "Skill 是面向使用者的受控入口/能力封装，自动化核心负责实际编排，MCP 是未来可供智能体调用的统一接口，不等同于核心本身。",
        a: `<p>“当前把 TESSY CLI 自动化流程封装成 Skill，方便工程师按统一方式提交任务；Skill 背后应调用同一套自动化核心，而不是自己再复制一套流程。自动化核心负责 Module、Analyze、范围、执行、报告、异常和恢复；CLI 和未来的 MCP 都只是接入方式。”</p><p>“当前已验证的是基础 CLI 闭环和简单 C/C++、Stub、Fault Injection、覆盖率与报告；统一编排、范围控制、实例锁和恢复正在建设，MCP 接入属于后续阶段。这样回答能把当前成果、建设中内容和规划边界分清。”</p>`,
        followups: [{ q: "为什么还需要 MCP？", a: "MCP 可以给智能体提供受约束、结构化的提交/查询工具，但不能绕过权限、范围和人工门禁；核心逻辑必须复用 CLI 流程。" }, { q: "Skill 失败时谁负责决定下一步？", a: "自动化核心返回阶段、日志、报告和可选动作；真实测试失败或需要修改范围时由工程师决策，不能让模型无限循环。" }],
        evidence: "TESSY CLI 架构文档：CLI/MCP 接入层与自动化核心分离",
        boundary: "不公开内部 Skill 名称、提示词、MCP 接口和服务器配置。",
        caution: "不要把 MCP 说成已经完成的产品能力。"
      },
      {
        id: "intern-ruiming-artifact-replace", priority: "must", difficulty: "advanced", tags: ["锐明技术", "so", "jsn", "js", "挂载", "验证"],
        q: "开发阶段挂载替换 .so、.jsn、.js 文件时，怎样避免验证结果失真？",
        brief: ".so 是 C++/头文件编译出的业务实现，.jsn 是参数默认值/范围配置，.js 是 Web 界面效果；替换验证必须记录版本、依赖、权限和最终正式产物。",
        a: `<p>“三类文件验证的关注点不同：替换 <code>.so</code> 要确认 ABI、依赖库、符号和加载路径；替换 <code>.jsn</code> 要检查 JSON 语法、默认值、范围、单位和旧配置兼容；替换 <code>.js</code> 要确认资源缓存、接口字段和 Web 状态刷新。每次挂载都记录原文件哈希、替换文件哈希、版本、配置和操作时间。”</p><p>“挂载验证通过后不能直接认为版本完成，还要把修改同步回源码/配置，做完整构建和安装包验证，确认正式产物与挂载文件行为一致，再上库。否则可能出现本地挂载有效、正式包漏文件或依赖不一致。”</p>`,
        followups: [{ q: ".so 能正常加载但功能不生效怎么办？", a: "检查实际加载路径、动态库依赖、符号版本、进程是否重启和旧库是否仍在内存；用日志或工具确认加载的文件哈希，而不是只看文件已替换。" }, { q: ".jsn 修改后前端值不对怎么查？", a: "先校验 JSON 和配置加载日志，再查默认值/范围校验、单位转换、缓存和页面绑定字段，最后确认后端实际收到的配置。" }],
        evidence: "本人确认的 .so/.jsn/.js 挂载替换验证流程",
        boundary: "不写入具体文件名、路径、客户参数和内部接口字段。",
        caution: "挂载是快速验证手段，不是替代编译、提测和上库的正式交付流程。"
      },
      {
        id: "intern-ruiming-bug-skill", priority: "must", difficulty: "advanced", tags: ["锐明技术", "Bug", "Skill", "定位", "修复"],
        q: "你们如何通过 Bug ID 自动分析和修复问题？",
        brief: "输入 Bug ID 后，Skill 拉取描述、复现条件和提交证据，建立版本/模块/文件上下文，分析根因并生成修复候选，再由开发自测、冒烟和回归确认。",
        a: `<p>“工程师不需要手工复制整段 Bug 内容，只需提供 Bug ID。Skill 先从缺陷系统拉取标题、期望/实际结果、版本、复现步骤、日志、截图和关联提交，再把这些内容整理成结构化问题卡片。随后结合代码仓库、配置和最近改动定位可能的模块与调用链，给出根因假设、证据、修改文件和验证建议；在权限和范围允许时生成修复补丁或辅助修改。”</p><p>“修复不能以模型输出结束：我会先看补丁是否落在正确的 SDK/Baseline/定制层，再用挂载替换快速复现，补充自测用例，做内部冒烟，最后按测试部 Bug 单回归并上库。每一步都保留 Bug ID、版本、证据、补丁和测试报告，方便审计。”</p>`,
        followups: [{ q: "怎样防止 Bug 描述误导 AI？", a: "把描述当作待验证输入，先独立复现并核对日志/版本/提交；对缺失或冲突信息列待确认项，不让模型直接跳到修改代码。" }, { q: "如何判断是关闭检测还是清除状态的问题？", a: "分别观察新事件是否继续产生、当前活动状态是否清空、UI/OSD 是否收到清除通知以及重启后是否恢复；不能只看配置开关值。" }, { q: "Skill 自动修复后谁负责合入？", a: "开发者负责审查补丁、运行自测和回归，测试部通过后再按团队流程上库；Skill 提供证据和候选修改，不替代代码评审。" }],
        evidence: "本人确认的 Bug ID→证据拉取→分析定位→修复流程",
        boundary: "不公开缺陷系统地址、Bug 编号、客户日志和内部 Skill 实现。",
        caution: "自动定位和生成补丁不等于根因已证明，回答中必须说清验证闭环。"
      },
      {
        id: "intern-ruiming-storage-alarm", priority: "must", difficulty: "advanced", tags: ["锐明技术", "存储报警", "Bug案例", "状态机"],
        q: "讲一下‘关闭存储报警后叠加报警信息仍存在’这个问题怎么排查？",
        brief: "关闭开关只阻止新存储报警识别，但没有清理已有活动报警和 Web 叠加层；需要沿配置→检测→状态→消息→界面逐层确认。",
        a: `<p><strong>建议按现象→假设→证据→修复回答：</strong></p><p>“测试部反馈关闭存储报警后，新的存储报警不再识别，但之前叠加在界面上的报警信息仍然存在。首先通过 Bug Skill 拉取 Bug 描述、复现步骤、版本和截图/日志，确认问题不是配置没有生效，而是‘禁止继续产生’和‘清除当前已显示状态’两个语义没有同时处理。”</p><p>“我沿着配置加载、存储状态检测、报警状态管理、消息/事件发布和 Web 叠加层刷新逐层打点：确认关闭开关后检测分支不再产生新事件；再检查已有 active alarm 是否仍保留、清除事件是否发布、前端是否收到并删除对应 overlay。合理的修复是在配置从启用变为关闭时清理对应活动状态并发布明确的 clear/disabled 事件，前端收到后移除叠加信息；同时保留其他类型报警，避免粗暴清空全部报警。”</p><p>“验证覆盖：先触发并显示存储报警，再关闭开关确认旧叠加消失且不再新增；切回开启后重新触发；同时存在其他报警时只清除存储报警；重启、配置持久化和挂载/正式包都要回归。最后把自测结果、Bug ID、版本和测试证据关联后上库。”</p>`,
        followups: [{ q: "为什么关闭开关后旧报警不会自动消失？", a: "开关可能只参与检测条件，阻止后续事件；已经进入活动列表、消息队列或前端 overlay 的状态需要显式清除和通知，不能假设它会自动回收。" }, { q: "如何证明不是前端缓存问题？", a: "同时看后端活动状态、发送给 Web 的事件和前端收到的字段；刷新页面/重启后对比。如果后端已清除且事件正确，才继续查前端状态缓存或 overlay 生命周期。" }, { q: "如何避免修复影响其他报警？", a: "使用报警类型/实例 ID 精确清除，只撤销存储报警对应状态和视图；回归其他报警叠加、同时触发和重复清除。" }],
        evidence: "本人确认的测试部 Bug 现象与 Skill 辅助定位流程",
        boundary: "具体修复文件和函数需以 Bug Skill 实际分析结果为准，不在题库中虚构文件名。",
        caution: "不要只回答‘把 if 条件改掉’，要说明活动状态、清除事件和前端叠加层的完整链路。"
      },
      {
